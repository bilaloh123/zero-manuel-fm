                ...
                {sessions.map((session) => (
                  <tr key={session.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-3 py-3">
                      {lotSessionIds.has(session.id) ? (
                        <span className="text-xs font-medium text-ink-muted">{t("lots.alreadyCreated")}</span>
                      ) : (
                        <button
                          type="button"
                          disabled={!session.end_time}
                          onClick={() => setLotSession(session)}
                          className="inline-flex items-center gap-1.5 rounded-control border border-border px-2.5 py-1 text-xs font-medium text-ink hover:bg-cream-soft disabled:cursor-not-allowed disabled:opacity-40"
                        >
