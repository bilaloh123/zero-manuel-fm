...
function LoginScreen({ onLogin }) {
  return (
    <div dir="rtl" style={{ background: `linear-gradient(160deg, ${c.headerGreenLight} 0%, ${c.headerGreen} 55%, #094433 100%)`, minHeight: "100vh", position: "relative", overflow: "hidden" }} className="flex flex-col items-center justify-center p-6">
      ...
      <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.8rem", position: "relative" }} className="mb-8">اختار حسابك للدخول (نسخة تجريبية)</p>
      <div className="flex flex-col gap-2.5 w-full" style={{ maxWidth: 340 }}>
        {users.map((u) => (
          <button key={u.id} onClick={() => onLogin(u)} style={{ background: "#fff", borderRadius: 15, padding: "13px 17px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }} className="flex items-center justify-between hover:opacity-95">
            <div className="flex items-center gap-3">
              <div style={{ background: c.bg, borderRadius: 10, width: 36, height: 36 }} className="flex items-center justify-center"><ShieldCheck size={16} color={c.cardGreen} /></div>
              <div className="text-right"><div style={{ fontWeight: 700, fontSize: "0.88rem" }}>{u.nom}</div><div style={{ fontSize: "0.68rem", color: c.inkMuted2 }}>{roleLabel[u.role]} · {u.farms.length > 1 ? "ضيعتين" : "ضيعة وحدة"}</div></div>
            </div>
            <ChevronLeft size={16} color={c.inkMuted2} />
          </button>
        ))}
      </div>
      <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.7rem", maxWidth: 300 }} className="mt-6 text-center">هادي نسخة تجريبية بلا كلمة سر. فالنسخة النهائية غادي يكون تسجيل دخول حقيقي بالهاتف/الإيميل.</p>
    </div>
  );
}
...
export default function App() {
  ...
  const [currentFarmId, setCurrentFarmId] = useState("zm");
  const [farms, setFarms] = useState(farmsInit);
  ...
  const data = farms[currentFarmId];
  ...
  if (!currentUser) {
    return <LoginScreen onLogin={(u) => { setCurrentUser(u); const fid = u.farms[0]; setCurrentFarmId(fid); setSelected(farms[fid].parcelles[0]); const perms = MODULES.filter((m) => permMatrix[u.role][m] !== "بلا وصول"); setTab(perms[0] || "لوحة"); }} />;
  }
  ...
  function switchFarm(fid) { setCurrentFarmId(fid); setSelected(farms[fid].parcelles[0]); }
  ...
  function addWorker() {
    ...
    updateFarm({ workers: [...data.workers, { id: Date.now(), nom: wForm.nom, parcelle: wForm.parcelle || data.parcelles[0].code, tache: wForm.tache || "—", type: wForm.type, dukhul: wForm.type === "ساعات" ? wForm.dukhul : "-", khuruj: wForm.type === "ساعات" ? wForm.khuruj : "-", qte, taux: Number(wForm.taux) || 0, moisQte: qte, dawra: wForm.dawra, statut: "غير مؤدى", audioNote: wForm.audioNote }] });
    ensureEmployee(wForm.nom);
  ...
  function addMyPointage() {
    const qte = wForm.type === "ساعات" ? hoursBetween(wForm.dukhul, wForm.khuruj) : Number(wForm.nahar) || 1;
    updateFarm({ workers: [...data.workers, { id: Date.now(), nom: currentUser.nom, parcelle: wForm.parcelle || data.parcelles[0].code, tache: wForm.tache || "—", type: wForm.type, dukhul: wForm.type === "ساعات" ? wForm.dukhul : "-", khuruj: wForm.type === "ساعات" ? wForm.khuruj : "-", qte, taux: Number(wForm.taux) || 15, moisQte: qte, dawra: wForm.dawra, statut: "غير مؤدى", audioNote: wForm.audioNote }] });
    ensureEmployee(currentUser.nom);
    setWForm({ nom: "", parcelle: "", tache: "", type: "ساعات", dukhul: "06:00", khuruj: "14:00", nahar: 1, taux: 15, dawra: "15", audioNote: "" });
    setShowAddWorker(false);
  }
  function removeWorker(id) { updateFarm({ workers: data.workers.filter((w) => w.id !== id) }); }
  function toggleStatut(id) {
    updateFarm({ workers: data.workers.map((w) => w.id === id ? { ...w, statut: w.statut === "مؤدى" ? "غير مؤدى" : "مؤدى" } : w) });
  }
    ...
    <div dir="rtl" style={{ background: c.bg, minHeight: "100vh", color: c.ink, fontFamily: "'Inter', sans-serif", paddingBottom: 80 }}>
      ...
      <header style={{ background: `linear-gradient(135deg, ${c.headerGreenLight} 0%, ${c.headerGreen} 100%)`, boxShadow: "0 4px 20px rgba(13,107,73,0.18)" }} className="px-5 py-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setCurrentUser(null)}><LogOut size={18} color="rgba(255,255,255,0.85)" /></button>
        </div>
      </header>
        ...
        {tab === "القطع" && (() => {
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2.5">
              {data.parcelles.map((p) => { const active = selected.code === p.code; return (
